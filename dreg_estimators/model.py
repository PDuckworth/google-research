...
def get_toy_models(train_xs, which_example="toy1D"):
  ...
  if FLAGS.dataset == "toy":
    ...
    with tf.name_scope("model_classes") as scope:
...
def iwae(p_z,           # prior
  ...
  with tf.name_scope('z_prior') as scope:
    prior = p_z(contexts)
    log_p_z = tf.reduce_sum(prior.log_prob(z), axis=-1)  # [num_samples, batch_size]

  with tf.name_scope('log_weights') as scope:
  ...
  if FLAGS.estimator == "bq":
      ...
      old_kernel = GPy.kern.RBF(FLAGS.latent_dim, lengthscale=[11.25066876, 0.59561975, 0.79575208, 19.29332173], variance=[0.7], ARD=True)
      # old_kernel = GPy.kern.RBF(FLAGS.latent_dim, lengthscale=[1]*FLAGS.latent_dim, variance=[0.7], ARD=True)
      ...
      def get_bq_estimate(loc, scale, observations, prior_samples_per_proposal):
          nonlocal old_kernel
          print("Old kernel:  ", old_kernel.lengthscale.values)
          ...
          with sess.as_default():
              ...
              gpy_gp = GPy.core.GP(reshape_x, reshape_y, kernel=old_kernel, likelihood=bq_likelihood, mean_function=mean_function)
              warped_gp = VanillaGP(gpy_gp)

              gpy_gp.optimize_restarts(num_restarts=2, verbose=False)

              # print(">> mu", gpy_gp.mean_function.mu)
              # print(">> m_0", gpy_gp.mean_function.m_0)
              # print(">> omega", gpy_gp.mean_function.omega)
              #
              # # # # Rescale the input x and y data
              # x_scaler = preprocessing.StandardScaler()
              # scaled_x = x_scaler.fit_transform(reshape_x)
              # # print("x scale", x_scaler.mean_, x_scaler.var_)
              # #
              # y_scaler = preprocessing.StandardScaler()
              # scaled_y = y_scaler.fit_transform(reshape_y)
              # # print("y scale", y_scaler.mean_, y_scaler.var_)
              # #
              # gpy_gp = GPy.core.GP(scaled_x, scaled_y, kernel=old_kernel, likelihood=bq_likelihood, mean_function=mean_function)
              # warped_gp = VanillaGP(gpy_gp)

              # gpy_gp.optimize_restarts(num_restarts=1, verbose=False)
              # old_kernel = kernel
              # print(">> scaled mu", gpy_gp.mean_function.mu)
              # print(">> scaled m_0", gpy_gp.mean_function.m_0)
              # print(">> scaled omega", gpy_gp.mean_function.omega)
              # print(">> lengthscales", gpy_gp.kern.lengthscale.values)

              # # Apply the rescale to the GP mean function
              # gpy_gp.mean_function.mu = x_scaler.transform(np.expand_dims(gpy_gp.mean_function.mu, axis = 0))
              # gpy_gp.mean_function.m_0 = y_scaler.transform(np.expand_dims(gpy_gp.mean_function.m_0, axis = 0))
              # gpy_gp.mean_function.omega = x_scaler.transform( 1/ np.expand_dims(gpy_gp.mean_function.omega, axis = 0))

          return gpy_gp.mean_function.mu, gpy_gp.mean_function.m_0, gpy_gp.mean_function.omega, gpy_gp.kern.lengthscale.values, gpy_gp.kern.variance.values[0], gpy_gp.X, warped_gp.underlying_gp.K_inv_Y

      with tf.name_scope('BQ_int') as scope:
          ...
          kernel_lengthscale.set_shape((FLAGS.latent_dim))  # (latent_dimensional)
      ...
      estimators['bq'] = (log_avg_weight, log_avg_weight, bq_loss, kernel_lengthscale)


  with tf.name_scope('analytic_kl') as scope:
